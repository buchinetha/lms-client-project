...
// 🧠 Submit Quiz (using Quiz collection)
app.post('/api/quiz/submit', async (req, res) => {
  try {
    ...
    // Save quiz results in Progress model
    let progress = await Progress.findOne({ studentId, courseId });
    if (!progress) {
      progress = new Progress({ studentId, courseId, completedLessons: [], quizResults: [] });
    }
...
// ✅ Get Quiz for a Course (use course.quiz directly)
app.get('/api/quiz/:courseId', async (req, res) => {
  try {
    ...
    res.json({ questions: course.quiz });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
